...
public class CoreSystemSetupUnitTest
{
	...
	private void verifyProductImport(final InOrder order, final String catalogName, final boolean sync)
	{
		order.verify(setupImpexService).importImpexFile(
				"/merchandisecore/import/productCatalogs/" + catalogName + "ProductCatalog/catalog.impex", true);
		...
		//
		//		order.verify(setupImpexService).importImpexFile(
		//				"/merchandisecore/import/productCatalogs/" + catalogName + "ProductCatalog/solrtrigger.impex", true);
	}
	...
	{
		order.verify(setupImpexService).importImpexFile(
				"/merchandisecore/import/contentCatalogs/" + catalogName + "ContentCatalog/catalog.impex", true);
		order.verify(setupImpexService).importImpexFile(
				"/merchandisecore/import/contentCatalogs/" + catalogName + "ContentCatalog/cms-content.impex", false);
		order.verify(setupImpexService).importImpexFile(
				"/merchandisecore/import/contentCatalogs/" + catalogName + "ContentCatalog/email-content.impex", false);
