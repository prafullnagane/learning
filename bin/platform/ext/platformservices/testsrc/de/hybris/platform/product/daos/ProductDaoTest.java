...
public class ProductDaoTest extends ServicelayerTransactionalTest
{
	...
	{
		final VariantType vt = VariantsManager.getInstance().createVariantType("VTTest");
		vt.createVariantAttributeDescriptor("color", TypeManager.getInstance().getType("java.lang.String"),
		...
		vt.createVariantAttributeDescriptor("size", TypeManager.getInstance().getType("java.lang.String"),
				AttributeDescriptor.READ_FLAG | AttributeDescriptor.WRITE_FLAG | AttributeDescriptor.OPTIONAL_FLAG
						| AttributeDescriptor.PROPERTY_FLAG);
		vt.createVariantAttributeDescriptor("ok", TypeManager.getInstance().getType("java.lang.Boolean"),
				AttributeDescriptor.READ_FLAG | AttributeDescriptor.WRITE_FLAG | AttributeDescriptor.OPTIONAL_FLAG
						| AttributeDescriptor.PROPERTY_FLAG);

		assertEquals(new HashSet<String>(Arrays.asList("color", "size", "ok")), variantsService.getVariantAttributes(vt.getCode()));

		final ProductModel base = productDao.findProductsByCode("testProduct0").get(0);
		base.setVariantType((VariantTypeModel) modelService.get(vt));

		modelService.save(base);

		assertEquals(Collections.EMPTY_LIST, base.getVariants());

		final VariantProductModel var1 = modelService.create(vt.getCode());
		var1.setCode("var1");
		var1.setBaseProduct(base);
		var1.setCatalogVersion(base.getCatalogVersion());

		final VariantProductModel var2 = modelService.create(vt.getCode());
		var2.setCode("var2");
		...
		assertEquals(vt.getCode(), var1.getItemtype());
		assertEquals(vt.getCode(), var2.getItemtype());
	...
	private boolean treeContainsCategoryWithCode(final CategoryModel c, final String categoryCode)
	{
		if (categoryCode.equals(c.getCode()))
		{
			return true;
		}
		final Collection<CategoryModel> supercategories = c.getSupercategories();
