...
public class DefaultCheckoutFacadeTest
{
	...
	public void setUp()
	{
		...
		try
		{
			given(commerceCheckoutService.placeOrder(cartModel)).willReturn(orderModel);
		}
	...
	public void testSetDeliveryAddressIfAvailable()
	{
		...
		verify(commerceCheckoutService).setDeliveryAddress(cartModel, addressModel);
	}
	...
	public void testNoSetDeliveryAddressIfAvailable()
	{
		...
		given(cartService.getSessionCart()).willReturn(cartModel);
		defaultCheckoutFacade.setDeliveryAddressIfAvailable();
		verify(commerceCheckoutService, Mockito.never()).setDeliveryAddress(cartModel, addressModel);
	}
	...
	{
		defaultCheckoutFacade.setDeliveryModeIfAvailable();
		verify(commerceCheckoutService, Mockito.never()).setDeliveryMode(cartModel, deliveryModeModel);
	}
	...
	public void testSetPaymentDetailsIfAvailable()
	{
		...
		verify(commerceCheckoutService).setPaymentInfo(cartModel, paymentInfoModel);
	}
	...
	public void testGetDeliveryCountries()
	{
		...
		verify(countryConverter, Mockito.times(2)).convert(Mockito.any(CountryModel.class));
	}
	...
	public void testSetPaymentDetails()
	{
		...
		verify(commerceCheckoutService).setPaymentInfo(cartModel, ccPaymentInfoModel);
	}
	...
	{
		defaultCheckoutFacade.setPaymentDetails("paymentId");
		verify(commerceCheckoutService, Mockito.never()).setPaymentInfo(Mockito.any(CartModel.class),
				Mockito.any(CreditCardPaymentInfoModel.class));
	}
	...
	public void testAuthorizePayment()
	{
		...
		given(commerceCheckoutService.authorizePayment(cartModel, "1234", provider)).willReturn(paymentTransactionEntryModel);
		defaultCheckoutFacade.authorizePayment("1234");
		verify(commerceCheckoutService).authorizePayment(cartModel, "1234", provider);
	}
	...
	public void testNoAuthorizePayment()
	{
		cartModel.setPaymentInfo(null);
		defaultCheckoutFacade.authorizePayment("1234");
		verify(commerceCheckoutService, Mockito.never()).authorizePayment(cartModel, "1234", "paymentProvider");
	}
	...
	{
		given(commerceCheckoutService.placeOrder(cartModel)).willReturn(null);
		defaultCheckoutFacade.placeOrder();
