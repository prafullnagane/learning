...
import de.hybris.platform.ycommercewebservices.dto.AddressDataList;
import de.hybris.platform.ycommercewebservices.dto.LoginChangeResponse;
import de.hybris.platform.ycommercewebservices.dto.PasswordRestoreResponse;
...
import de.hybris.platform.ycommercewebservices.user.data.ExtendedCustomerData;
...
public class CustomersController extends BaseController
{
	...
	@ResponseBody
	public ExtendedCustomerData updateProfile(final HttpServletRequest request) throws DuplicateUidException
	{
		...
		return convert(customerFacade.getCurrentCustomer());
	}
	...
	@ResponseBody
	public AddressData createAddress(final HttpServletRequest request) throws InconsistentUseridException, DuplicateUidException,
	...
	@ResponseBody
	public ExtendedCustomerData getCurrentCustomer() throws InconsistentUseridException
	{
		return convert(customerFacade.getCurrentCustomer());
	}
	...
	@Secured("ROLE_CUSTOMERGROUP")
	@RequestMapping(value = "/current/login", method = RequestMethod.POST)
	@ResponseBody
	...
	private ExtendedCustomerData convert(final CustomerData source)
	{
		final ExtendedCustomerData extendedCustomerData = new ExtendedCustomerData();
		extendedCustomerData.setCurrency(source.getCurrency());
		extendedCustomerData.setDefaultBillingAddress(source.getDefaultBillingAddress());
		extendedCustomerData.setDefaultShippingAddress(source.getDefaultShippingAddress());
		extendedCustomerData.setDisplayUid(source.getDisplayUid());
		extendedCustomerData.setFirstName(source.getFirstName());
		extendedCustomerData.setLanguage(source.getLanguage());
		extendedCustomerData.setLastName(source.getLastName());
		extendedCustomerData.setName(source.getName());
		extendedCustomerData.setTitleCode(source.getTitleCode());
		extendedCustomerData.setUid(source.getUid());
		if (source.getTitleCode() != null)
		{
			...
			if (titles != null)
			{
				...
				{
					if (source.getTitleCode().equals(titleData.getCode()))
					{
						extendedCustomerData.setTitle(titleData.getName());
						break;
					}
				}
			}
		}
		return extendedCustomerData;
	}
}
