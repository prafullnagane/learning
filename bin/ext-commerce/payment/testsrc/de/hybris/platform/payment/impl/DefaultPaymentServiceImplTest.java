...
public class DefaultPaymentServiceImplTest
{
	...
	public void testCodeGeneration()
	{
		...
		final PaymentTransactionEntryModel paymentTransaction = defaultPaymentServiceImpl.authorize("MTC", new BigDecimal(11),
				Currency.getInstance("USD"), new AddressModel(), new AddressModel(), new CardInfo());


		final CurrencyModel curr = new CurrencyModel();
		curr.setIsocode("USD");
		paymentTransaction.setCurrency(curr);
		paymentTransactionModel.setEntries(Arrays.asList(paymentTransaction));

		Assertions.assertThat(paymentTransaction.getCode()).isEqualTo("MTC-1");

		final PaymentTransactionEntryModel paymentTransaction2 = defaultPaymentServiceImpl.capture(paymentTransactionModel);


		Assertions.assertThat(paymentTransaction2.getCode()).isEqualTo("MTC-2");
	}
}
