	...
	protected void createUpdatePickupGroupData(final AbstractOrderEntryModel entryModel, final AbstractOrderData target)
	{
		...
		if (pointOfServiceModel != null)
		{
			...
			if (groupData == null)
			{
				...
				if (pointOfServiceData.getDistanceKm() == null && getCustomerLocationService().getUserLocation() != null)
				{
					pointOfServiceData.setDistanceKm(Double.valueOf(getCustomerLocationService().calculateDistance(
							getCustomerLocationService().getUserLocation().getPoint(), pointOfServiceModel)));
					calculateFormattedDistance(pointOfServiceModel, pointOfServiceData);
				}
