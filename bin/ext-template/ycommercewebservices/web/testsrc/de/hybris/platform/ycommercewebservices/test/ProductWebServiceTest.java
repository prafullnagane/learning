...
public class ProductWebServiceTest
{
	...
	private static final String URL = "http://electronics.local:9001/rest/v1/products";
	...
	public void testGetProductByCode_Success_XML_Deep()
	{
		...
		final ProductData p = response.getBody();



		assertEquals(TestConstants.PRODUCT_CODE, p.getCode());
		assertEquals("EASYSHARE V1253, Black", p.getName());
		assertEquals(5, p.getImages().size());
	}
	...
	public void testGetProductByCode_Success_XML_Options_Description()
	{
		...
		assertTrue("Description may not be null!", p.getDescription() != null);
	}
	...
	public void testGetProductByCode_Success_XML_Options_Gallery()
	{
		...
		assertEquals(TestConstants.PRODUCT_CODE, p.getCode());
		assertEquals("EASYSHARE V1253, Black", p.getName());
		assertEquals(10, p.getImages().size());
	...
	public void testGetProductByCode_Success_XML_Options_Review()
	{
		...
		assertEquals(TestConstants.PRODUCT_CODE, p.getCode());
		assertEquals("EASYSHARE V1253, Black", p.getName());
		assertEquals(0, p.getReviews().size());
	...
	public void testPutProductByCode_Success_XML_Options_Review()
	{
		...
		final ReviewData p = response.getBody();
	...
	public void testGetProductByCode_Success_XML_Options_Classifications()
	{
		...
		assertEquals(TestConstants.PRODUCT_CODE, p.getCode());
		assertEquals("EASYSHARE V1253, Black", p.getName());
		assertEquals(23, p.getClassifications().size());
	...
	public void testBasicSearch2()
	{
		...
		final FacetSearchPageData<SolrSearchQueryData, ProductData> d = response.getBody();
		assertTrue(d != null);

		assertEquals("a", d.getCurrentQuery().getFreeTextSearch());
		assertEquals(0, d.getBreadcrumbs().size());
		assertTrue(d.getFacets() != null);
		assertEquals(20, d.getResults().size());
		assertEquals(6, d.getSorts().size());

		for (final ProductData p : d.getResults())
		{
	...
	public void testSearchRefine()
	{
		...
		final FacetData<SolrSearchQueryData> facet = d.getFacets().get(0);
