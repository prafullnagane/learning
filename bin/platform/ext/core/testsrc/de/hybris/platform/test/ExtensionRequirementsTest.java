...
public class ExtensionRequirementsTest extends HybrisJUnit4Test
{
	...
	public void testEclipseClasspathMatchesExtensionInfoXml() throws Exception
	{
		...
		{
			// skip the platform extensions since they don't really need proper requirements or if explicitly excluded
			if (platformExtensions.contains(extension) || excludedExtensions.contains(extension))
			{
				continue;
			}
	...
	public void testCorrectPlatformClasspathEntry() throws Exception
	{
		...
		for (final ExtensionInfo extension : includedExtensions)
		{
			// skip the platform extensions since they don't really need proper requirements
			if (platformExtensions.contains(extension) || excludedExtensions.contains(extension))
			{
				continue;
			}
	...
	public void testIllegalWebExports() throws Exception
	{
		...
		for (final ExtensionInfo extension : includedExtensions)
		{
			...
			final EclipseClasspath eclipseClasspath = EclipseClasspathReader.readClasspath(extension);
			for (final ClasspathEntry classpathEntry : eclipseClasspath.getAllClasspathEntries())
			{
