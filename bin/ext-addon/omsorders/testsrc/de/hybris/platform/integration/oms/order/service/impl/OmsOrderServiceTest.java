...
public class OmsOrderServiceTest
{
	...
	public void init()
	{
		...
		defaultOmsOrderService.setRestClient(omsOrdersRestClient);
		Mockito.when(order.getCode()).thenReturn("order1");
		Mockito.when(modelService.create(CsTicketModel.class)).thenReturn(ticket);
		Mockito.when(orderConverter.convert(order)).thenReturn(convertedOrder);
	}
	...
	public void shouldCreateOmsOrder()
	{
		...
		Mockito.verify(omsOrdersRestClient).createOrder(convertedOrder);
	...
	public void shouldNotCreateOmsOrder()
	{
		...
		Mockito.doThrow(new RestCallException(restResponse)).when(omsOrdersRestClient).createOrder(convertedOrder);
		...
		Mockito.verify(omsOrdersRestClient).createOrder(convertedOrder);
		//Mockito.verify(order).setExportToOmsCanBeRetried(Boolean.TRUE);
