...
public class Order2PromotionResultRelationTest extends AbstractPromotionServiceTest
{
	 ...
	 * {@link PromotionResultModel} and {@link AbstractPromotionActionModel}. See PRO-66 for more details.
	 * <ul>
	...
	public void testOrderPromotionResultRelationTest() throws CalculationException
	{
		...
		try
		{
			...
			//copied order, so each table size is doubled
			testTableSize(4, 4);
		}
	...
	private void testTableSize(final int promotionResultSize, final int promotionActionSize)
	{
		assertEquals("wrong promotion result size", promotionResultSize, getTableSize(PromotionResultModel._TYPECODE));
		assertEquals("wrong promotion action size", promotionActionSize, getTableSize(AbstractPromotionActionModel._TYPECODE));
	}
