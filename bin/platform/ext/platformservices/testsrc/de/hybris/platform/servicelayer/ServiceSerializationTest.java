...
public class ServiceSerializationTest extends ServicelayerTransactionalTest
{
	...
	private void verifyListSize(final List l)
	{
		Assert.assertTrue("list is null ", l != null);
		Assert.assertTrue(l.getClass().getName() + " list is empty", !l.isEmpty());
		Assert.assertTrue(l.getClass().getName() + " list does not contain expected " + EXPECTED_PRODUCTS + " items ",
				l.size() == EXPECTED_PRODUCTS);
	}
	...
	private LazyLoadModelList getList()
	{
		...
		q.setResultClassList(Arrays.asList(String.class, String.class, Unit.class));
	...
	private byte[] serializeObject(final Object s) throws IOException
	{
		LOG.info("serializing object " + s);
