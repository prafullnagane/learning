...
public class DBPersistenceManagerConcurrencyTest extends HybrisJUnit4Test
{
	...
	{
		testConcurrentAccess(50, 120 * MULTIPLICATOR, false, false);
	}

	@Test
	public void testConcurrentAccessNotLoaded()
	{
		testConcurrentAccess(50, 120 * MULTIPLICATOR, true, false);
	}

	@Test
	public void testConcurrentAccessCleared()
	{
		testConcurrentAccess(50, 120 * MULTIPLICATOR, false, true);
	}
			...
			final int THREADS, final boolean clearType, final boolean clearComplete)
	{
		...
		assertTrue("not all threads ended normally", threads.stopAndDestroy(15 * MULTIPLICATOR));

		final Map<Integer, Throwable> errors = threads.getErrors();
		if (!errors.isEmpty())
		{
			for (final Map.Entry<Integer, Throwable> e : errors.entrySet())
			{
				System.err.println("got error in runner " + e.getKey() + ":");
				e.getValue().printStackTrace();
			}
			fail("got " + errors.size() + " runner errors - see console");
		}
