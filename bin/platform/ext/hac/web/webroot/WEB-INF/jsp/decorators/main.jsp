		...
		<div id="mainContainer" class="container" data-homeLink="<c:url value="/"/>" data-contextPath="<%= request.getContextPath() %>">
			<header class="span-24 last">
				<c:if test="${requestScope.initialized == true}">
					<div id="loginInfo" onclick="location.href='<c:url value="/j_spring_security_logout"/>';"> 
						<spring:message code="general.credentials.welcome" /> ${jalosession.user.name != null ? jalosession.user.name : jalosession.user.uid} - logout					
					</div>
				</c:if>
			...
			<div id="notification" class="span-24 last" data-hurl="${statisticsPayload.homeURL}" data-pl="${statisticsPayload.encodedData}" data-lid="${statisticsPayload.licenseId}">
