...
public class DynamicComparatorTest
{
	...
	{
		final DynamicComparator dc = new DynamicComparator("name");
		final TestClass biggerInstance2 = new TestClass("bbbb", 3);

		assertTrue("Invalid comparison result:", dc.compare(smallerInstance, biggerInstance) < 0);
		assertTrue("Invalid comparison result:", dc.compare(biggerInstance, smallerInstance) > 0);
		assertTrue("Invalid comparison result:", dc.compare(biggerInstance, biggerInstance2) == 0);
	...
	{
		final DynamicComparator dc = new DynamicComparator("color");
		assertTrue("Invalid comparison result:", dc.compare(smallerInstance, biggerInstance) < 0);
	}
	...
	{
		final DynamicComparator dc = new DynamicComparator("size");
		assertTrue("Invalid comparison result:", dc.compare(smallerInstance, biggerInstance) < 0);
	}
	...
	{
		final DynamicComparator dc = new DynamicComparator("aaaa");
	...
	{
		final DynamicComparator dc = new DynamicComparator("nickName");
	...
	public static class ComparableTestClass implements Comparable<ComparableTestClass>
	{
		...
		@Override
		public int compareTo(final ComparableTestClass o)
		{
