...
rem version: 2.6.3.RELEASE
rem build date: 20111212200713

setlocal enabledelayedexpansion

set TCRUNTIME_VER=2.6.3.RELEASE
...
echo INFO Running Windows architecture: %ARCH%

set tomcat_bin=%CATALINA_BASE%\bin\%ARCH%\wrapper.exe
set wrapper_conf=%CATALINA_BASE%\conf\wrapper.conf
if /i "%action%" == "run_minimal" (
set wrapper_conf=%CATALINA_BASE%\conf\wrapper-minimal.conf
)
if /i "%action%" == "start_minimal" (
set wrapper_conf=%CATALINA_BASE%\conf\wrapper-minimal.conf
)
set default_flags=
...
echo INFO Script directory:    %SCRIPT_PATH%
echo INFO tc Runtime location: %_REALPATH%
...
if /i "%action%" == "start" (
...
) else if /i "%action%" == "start_minimal" (
    echo INFO Starting instance at %CATALINA_BASE%
    "%tomcat_bin%" -t "%wrapper_conf%" %wrapper_update0% "%wrapper_update1%" "%wrapper_update2%" "%wrapper_update3%" %wrapper_update4% %wrapper_update5% %wrapper_update6%
    ...
    echo INFO Restarting instance at %CATALINA_BASE%
    "%tomcat_bin%" -p "%wrapper_conf%" %wrapper_update0% "%wrapper_update1%" "%wrapper_update2%" "%wrapper_update3%" %wrapper_update4% %wrapper_update5% %wrapper_update6% wrapper.shutdown.timeout=%stop_timeout%
    "%tomcat_bin%" -t "%wrapper_conf%" %wrapper_update0% "%wrapper_update1%" "%wrapper_update2%" "%wrapper_update3%" %wrapper_update4% %wrapper_update5% %wrapper_update6%
  ...
  echo     start_minimal     - starts the Windows service for this instance with minimal configuration
  echo     run               - starts an instance as a foreground process
  echo     run_minimal       - starts an instance as a foreground process with minimal configuration
