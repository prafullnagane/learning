...
public class DefaultProductDao extends DefaultGenericDao<ProductModel> implements ProductDao
{
	...
	public SearchResult<ProductModel> findProductsByCategory(final CategoryModel category, final int start, final int count)
	{
		...
		sb.append("SELECT {p:").append(ProductModel.PK).append("} ");
		sb.append("FROM {").append(ProductModel._TYPECODE).append(" AS p ");
		sb.append("JOIN ").append(CategoryConstants.Relations.CATEGORYPRODUCTRELATION).append(" AS l ");
		sb.append("ON {l:").append(LinkModel.TARGET).append("}={p:").append(ProductModel.PK).append("} } ");
