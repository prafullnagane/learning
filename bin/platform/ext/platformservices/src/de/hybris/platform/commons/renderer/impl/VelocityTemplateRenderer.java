...
public class VelocityTemplateRenderer implements Renderer
{
	...
	public void render(final RendererTemplateModel template, final Object context, final Writer output)
	{
		...
		try
		{
			c = Class.forName(template.getContextClass());
		}
		...
		try
		{
			if ((context != null) && (!c.isAssignableFrom(context.getClass())))
			{
			...
			is = mediaService.getStreamFromMedia(content);

			writeToOutput(output, is, context);
		}
	...
	private void writeToOutput(final Writer result, final InputStream is, final Object context) throws IOException
	{
