...
public abstract class AbstractLockHandlerIntegrationTest extends HybrisJUnit4Test
{
	...
	{
		final TestInitHandler h = (TestInitHandler) handler;
		try
		{
			System.err.println("------------------------------------------------");
			final JdbcTemplate jdbcTemplate = new JdbcTemplate(h.dataSource);
			jdbcTemplate.execute("DROP TABLE  " + h.tableName);
		}
		catch (final DataAccessException dae)
		{
			LOG.warn(" Table " + h.tableName + " was not created during the test ");
	...
	{
		final TestInitHandler h = (TestInitHandler) handler;
		return checkTestTableExists(h.dataSource, h.tableName);
	}

	protected boolean checkTestTableExists(final DataSource ds, final String tableName)
	{
		try
		{
