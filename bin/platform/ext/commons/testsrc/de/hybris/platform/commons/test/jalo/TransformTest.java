...
public class TransformTest extends HybrisJUnit4TransactionalTest
{
	...
	private String getStringFromStream(final InputStream is) throws Exception
	{
	...
	public void testSmallTest() throws Exception
	{
		...
		InputStream xslStream = getStream("/commons/unittest/simpleTransformation.xsl");
		final String stringStringResult = CommonsManager.getInstance().transform(html, xslStream, true);
		xslStream = getStream("/commons/unittest/simpleTransformation.xsl");
		final String streamStringResult = CommonsManager.getInstance().transform(htmlStream, xslStream, true);
