...
abstract public class AbstractHybrisNumberValidatorTest<T extends Number, A extends java.lang.annotation.Annotation>
{
	...
	private A prepareAnnotation(final boolean max)
	{
		...
		try
		{
			...
			final StringMemberValue mv = (StringMemberValue) idAnnotation.createMemberValue(clazz.getClassFile().getConstPool(),
					ClassPool.getDefault().get(String.class.getName()));
			mv.setValue(max ? getBorderCaseMax().toString() : getBorderCaseMin().toString());
			idAnnotation.addMemberValue("value", mv);

			attribute.addAnnotation(idAnnotation);

			final CtField f = new CtField(CtClass.longType, "field", clazz);
