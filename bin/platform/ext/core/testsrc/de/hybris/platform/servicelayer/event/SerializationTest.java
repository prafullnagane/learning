...
public class SerializationTest extends MockTest
{
	...
	@Resource(name = "jBossSerializationService")
	private SerializationService jBossSerializationService;
	...
	public void testSize()
	{
		...
		LOG.info("=== JBossSerializationService ===");
		test(jBossSerializationService, true);
	}
	...
	public void testPerformance()
	{
		...
		stopwatch.stop();
		stopwatch = new StopWatch("jbossSerializationService");
		for (int i = 0; i < CNT; i++)
		{
			test(jBossSerializationService, false);
		}
		stopwatch.stop();


		stopwatch = new StopWatch("customEventWithJBossSerialization");
		for (int i = 0; i < CNT; i++)
		{
			jBossSerializationService.deserialize(jBossSerializationService.serialize(new CustomTxEvent(true, "testid")));
		}
	...
	{
		final byte[] byt = jBossSerializationService.serialize(new CustomTxEvent(true, "testid"));
	...
	{
		final StopWatch stopwatch = new StopWatch("customEventWithJBossSerializationAndBase64");
		for (int i = 0; i < CNT; i++)
		{
			byte[] byt = jBossSerializationService.serialize(new CustomTxEvent(true, "testid"));
			final String str = Base64.encodeBytes(byt, Base64.DONT_BREAK_LINES);
			byt = Base64.decode(str);
			jBossSerializationService.deserialize(byt);
		}
		final long time = stopwatch.stop();
		LOG.info("serializations/second=" + CNT / ((double) time / 1000 / 1000 / 1000));
	}
