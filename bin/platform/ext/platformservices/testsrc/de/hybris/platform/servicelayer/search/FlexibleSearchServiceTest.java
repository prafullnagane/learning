...
public class FlexibleSearchServiceTest extends ServicelayerTransactionalTest
{
	...
	public void testSearchWithSingleParamsForLanguage() throws ConsistencyCheckException
	{
		...
		try
		{
			l1 = C2LManager.getInstance().getLanguageByIsoCode("falseLang");
		}
		catch (final JaloItemNotFoundException jile)
		{
			l1 = C2LManager.getInstance().createLanguage("falseLang");
		}
		l1.setActive(false);
	...
	public void testMultiColumnSearch()
	{
		...
		try
		{
			...
			final TitleModel[] t = new TitleModel[max];

			for (int i = 0; i < max; i++)
			{
				t[i] = new TitleModel();
				t[i].setCode(("t" + (i / 10)) + (i % 10));
				t[i].setName("t" + i + "name", i18nService.getCurrentLocale());
			}

			modelService.saveAll(Arrays.asList(t));

			final FlexibleSearchQuery q = new FlexibleSearchQuery(//
					"SELECT {code},{PK},{name} FROM {Title} WHERE {PK} IN (?titles) ORDER BY {code} ASC");
			q.addQueryParameter("titles", Arrays.asList(t));
			q.setResultClassList(Arrays.asList(String.class, Title.class, String.class));

			final SearchResult<List<?>> sr = flexibleSearchService.search(q);
			assertEquals(max, sr.getTotalCount());
			final List list = (List) ModelUtils.getFieldValue(sr, "resultList");
			assertNotNull(list);
			...
			for (int i = 0; i < max; i++)
			{
				...
				assertEquals(t[i].getCode(), row.get(0));
				assertEquals(t[i], row.get(1));
				assertEquals(t[i].getName(), row.get(2));
			}
		}
		finally
		{
	...
	protected void randomizeStatuses(final ArticleApprovalStatus... statuses)
	{
		...
		for (final ProductModel product : resultListProduct)
		{
			try
			{
				product.setApprovalStatus(enumVals.get(i++ % enumVals.size()));
	...
	{
		final String query = "select distinct({passwordanswer}) from {customer}"; //this query (should) returns a single row with NULL

		//first jalo search - so should it be
		final List<?> jaloResultList = FlexibleSearch.getInstance().search(query, Object.class).getResult();
		assertEquals(1, jaloResultList.size());
	...
	public void testTranslate()
	{
		...
		final String sqlQuery = "SELECT  item_t0.PK  FROM " + ("master".equals(tenantID) ? "" : tenantID + "_")
		...
		translationResult = flexibleSearchService.translate(searchQuery);
		...
		assertEquals(2, translationResult.getSQLQueryParameters().size());
		assertEquals(Boolean.TRUE, translationResult.getSQLQueryParameters().get(0));
	}
	...
	public void testTranslateWithCustomSession()
	{
		...
		final Set<PK> catalogVersionPKSet = new HashSet<PK>();
		for (final CatalogModel cmodel : catalogService.getAllCatalogs())
		{
			final List<CatalogVersionModel> cvModelList = searchQuery.getCatalogVersions() == null ? new ArrayList<CatalogVersionModel>()
					: new ArrayList<CatalogVersionModel>(searchQuery.getCatalogVersions());
