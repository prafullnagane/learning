...
public class Europe1DiscountsTranslatorTest extends HybrisJUnit4TransactionalTest
{
	...
	public void testGlobalDiscountRowOverallImport()
	{
		...
		try
		{
			final User u1 = (User) ir.readLine();
			assertNotNull(u1);
		...
		try
		{
			final User u2 = (User) ir.readLine();
			assertNotNull(u2);
		...
		try
		{
			final User u3 = (User) ir.readLine();
			assertNotNull(u3);
		...
		try
		{
			final User u4 = (User) ir.readLine();
			assertNotNull(u4);
	...
	private void checkDiscountRowValue(final NumberFormat nf, final String valueAsString, final AbstractDiscountRow adr)
	{
		try
		{
			assertEquals(adr.getValue().doubleValue(), nf.parse(valueAsString).doubleValue(), 0.00);
		}
