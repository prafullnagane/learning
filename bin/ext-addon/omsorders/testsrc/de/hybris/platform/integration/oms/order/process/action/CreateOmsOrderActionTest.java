...
public class CreateOmsOrderActionTest
{
	...
	public void shouldSetTheOrderToRetry() throws Throwable
	{
		...
		try
		{
			Mockito.doThrow(new RestCallException(restResponse)).when(impersonationService)
	...
	@Test
	public void shouldSetTheOrderToNotRetry() throws Throwable
	{
		TestUtils.disableFileAnalyzer("Here an exception is expectred");
		try
		{
			Mockito.doThrow(new RestCallException(restResponse)).when(impersonationService)
					.executeInContext(Mockito.any(ImpersonationContext.class), Mockito.any(ImpersonationService.Executor.class));
			Mockito.when(restResponse.getStatus()).thenReturn(Status.CONFLICT);
			Mockito.when(Integer.valueOf(restResponse.getStatusCode())).thenReturn(Integer.valueOf(409));
			final Transition transition = action.executeAction(orderProcessModel);
	...
	@Test
	public void shouldSetTheOrderToNotExported() throws Throwable
	{
		TestUtils.disableFileAnalyzer("Here an exception is expectred");
		try
		{
			Mockito.doThrow(new RuntimeException()).when(impersonationService)
					.executeInContext(Mockito.any(ImpersonationContext.class), Mockito.any(ImpersonationService.Executor.class));
			Mockito.when(Integer.valueOf(restResponse.getStatusCode())).thenReturn(Integer.valueOf(409));
			final Transition transition = action.executeAction(orderProcessModel);
			Assert.assertEquals(Transition.NOK, transition);
