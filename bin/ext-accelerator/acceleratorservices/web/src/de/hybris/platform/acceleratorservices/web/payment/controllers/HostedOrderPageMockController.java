...
public class HostedOrderPageMockController
{
	...
	protected Map<String, String> deserializeParameterMap(final String paramString)
	{
		...
		final String[] strings = paramString.split(SEPARATOR_STR);
		for (int i = 0; i < strings.length; i += 2)
		{
			results.put(strings[i], strings[i + 1]);
		}
		return results;
	}
	...
	public static class CountryComparator extends AbstractComparator<SelectOption>
	{
		...
		protected int compareInstances(final SelectOption option1, final SelectOption option2)
		{
			...
			if (option1.getName() == null && option2.getName() == null)
			{
				result = BEFORE;
			}
			else
			{
				result = option1.getName().compareToIgnoreCase(option2.getName());
			}

			if (EQUAL == result)
			{
				result = option1.getCode().compareToIgnoreCase(option2.getCode());
			}
			return result;
		}
	}
