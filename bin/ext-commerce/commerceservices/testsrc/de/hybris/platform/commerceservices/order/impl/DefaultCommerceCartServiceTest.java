...
public class DefaultCommerceCartServiceTest
{
	...
	@Mock
	private CartModel oldCart, newCart;
	@Mock
	...
	@Mock
	private CalculationService calculationService; //NOPMD
	@Mock
	private ExternalTaxesService externalTaxesService; //NOPMD
	@Mock
	...
	@Mock
	private PromotionsService promotionsService; //NOPMD
	@Mock
	...
	public void setUp() throws Exception
	{
		...
		stubCartModel();
		commerceCartService.setCartValidityPeriod(0);
		commerceCartService.setGuidKeyGenerator(guidKeyGenerator);
	...
	private void stubOrder()
	{
		...
		given(orderEntryModel.getDeliveryPointOfService()).willReturn(storeOne);
		given(orderEntryModel.getQuantity()).willReturn(Long.valueOf(STORE_ONE_STOCK_QUANTITY));
		given(orderEntryModel.getUnit()).willReturn(unitModel);
	}
	...
	{
		given(oldCart.getSite()).willReturn(baseSiteModel);
		given(oldCart.getEntries()).willReturn(Collections.singletonList(orderEntryModel));
	...
	{
		commerceCartService.addToCart(cartModel, null, 1, unitModel, false);
	}
	...
	public void testAddToCartWhenProductIsBase() throws CommerceCartModificationException
	{
		...
		commerceCartService.addToCart(cartModel, productModel, 1, unitModel, false);
	}
	...
	public void testAddToCartWhenQuantityIsZero() throws CommerceCartModificationException
	{
		...
		commerceCartService.addToCart(cartModel, productModel, 0, unitModel, false);
	}
	...
	public void testAddToCartWhenNoUnitFound() throws CommerceCartModificationException
	{
		...
		commerceCartService.addToCart(cartModel, productModel, 1, null, false);
	...
	public void testAddToCartLowerStock() throws CommerceCartModificationException, CalculationException
	{
		...
		final long actualAdded = commerceCartService.addToCart(cartModel, productModel, 20, unitModel, false).getQuantityAdded();
	...
	public void testAddToCart() throws CommerceCartModificationException, CalculationException
	{
		given(defaultCommerceStockService.getStockLevelForProductAndBaseStore(productModel, baseStoreModel)).willReturn(
				Long.valueOf(10));
		given(productModel.getMaxOrderQuantity()).willReturn(null);
		final long actualAdded = commerceCartService.addToCart(cartModel, productModel, 1, unitModel, false).getQuantityAdded();
	...
	{
		commerceCartService.calculateCart(null);
	}
	...
	{
		commerceCartService.recalculateCart(null);
	}
	...
	{
		commerceCartService.removeAllEntries(null);
	}
	...
	public void testRemoveAllEntries()
	{
		...
		commerceCartService.removeAllEntries(cartModel);
	}
	...
	public void testGetEntryForNumberEmptyEntries()
	{
		...
		final AbstractOrderEntryModel entry = commerceCartService.getEntryForNumber(cartModel, 2);
	...
	public void testCheckCartLevelWithEntries()
	{
		...
		final long level = commerceCartService.checkCartLevel(productModel, cartModel, null);
	...
	public void testCheckStockLevelEmptyWarehouse()
	{
		given(Boolean.valueOf(defaultCommerceStockService.isStockSystemEnabled(Mockito.any(BaseStoreModel.class)))).willReturn(
				Boolean.FALSE);
		final long stock = commerceCartService.getAvailableStockLevel(productModel, null);
	...
	public void testCanModifyCartEntry() throws CommerceCartModificationException
	{
		...
		Assert.assertNotNull(commerceCartService.updateQuantityForCartEntry(cartModel, FIRST, 100));
	}
	...
	public void testCanNotModifyCartEntry()
	{
		...
		try
		{
			commerceCartService.updateQuantityForCartEntry(cartModel, FIRST, 100);
	...
	public void testGetAvailableStockLevelFromPos()
	{
		...
		assertEquals("Unexpected stock quantity for the point of service: storeOne", STORE_ONE_STOCK_QUANTITY,
				commerceCartService.getAvailableStockLevel(productModel, storeOne));
		assertEquals("Unexpected stock quantity for the point of service: storeTwo", STORE_TWO_STOCK_QUANTITY,
				commerceCartService.getAvailableStockLevel(productModel, storeTwo));
	}
	...
	public void testCartHasBeenRecentlyUpdated() throws CommerceCartRestorationException
	{
		...
		given(oldCart.getModifiedtime()).willReturn(new Date(1L));
		given(timeService.getCurrentTime()).willReturn(new Date(0L));

		assertFalse("Old cart should not equal the new cart", oldCart.equals(newCart));
		assertEquals("No modifications should be present", 0, commerceCartService.restoreCart(oldCart).getModifications().size());
	}
	...
	public void testCartHasNotBeenRecentlyUpdated() throws CommerceCartRestorationException
	{
		...
		given(oldCart.getModifiedtime()).willReturn(new Date(0L));
		...
		assertFalse("Old cart should not equal the new cart", oldCart.equals(newCart));
		assertEquals("One modification should be present", 1, commerceCartService.restoreCart(oldCart).getModifications().size());
	}
	...
	public void testAddToCartStockFromPos() throws CommerceCartModificationException
	{
		...
		final CommerceCartModification modification = commerceCartService.addToCart(cartModel, productModel, storeOne,
				NUMBER_OF_ITEMS_TO_CART, unitModel, false);
	...
	public void testUpdateQuantityForPickupCartEntry() throws CommerceCartModificationException
	{
		...
		CommerceCartModification commerceCartModification = commerceCartService.updateQuantityForCartEntry(cartModel, 1, 1000);
		assertEquals(10, commerceCartModification.getQuantityAdded());

		commerceCartModification = commerceCartService.updateQuantityForCartEntry(cartModel, 1, 5);
		assertEquals(5, commerceCartModification.getQuantityAdded());

		commerceCartService.updateQuantityForCartEntry(cartModelWithoutPosEntries, 2, 5);
	}
	...
	public void testUpdatePointOfServiceForCartEntry() throws CommerceCartModificationException
	{
		...
		given(commerceCartService.getEntryForProductAndPointOfService(cartModel, productModel1, pointOfServiceModel1)).willReturn(
				Integer.valueOf(0));
		given(commerceCartService.getEntryForProductAndPointOfService(cartModel, productModel1, pointOfServiceModel2)).willReturn(
				Integer.valueOf(1));

		//Update the POS for the second entry with POS1 so that it gets merged with existing entry
		final CommerceCartModification modification = commerceCartService.updatePointOfServiceForCartEntry(cartModel, 1,
				pointOfServiceModel1);
	...
	public void testUpdateToShippingModeForCartEntry() throws CommerceCartModificationException
	{
		...
		final CommerceCartModification modification = commerceCartService.updateToShippingModeForCartEntry(cartModel, 0);
		...
		final CommerceCartModification modification2 = commerceCartService.updateToShippingModeForCartEntry(cartModel, 0);
	...
	public void shouldAddToCartAdjustedQuantityWhenMaxOrderQuantityForProductExceeded() throws Exception
	{
		...
		final CommerceCartModification cartModificationResult = commerceCartService.addToCart(cartModel, productModel, 8,
				unitModel, false);
