...
public class SearchResultTest extends HybrisJUnit4Test
{
	...
	public void testResults()
	{
		...
		{
			final Title t = (Title) row.get(3);
			Assert.assertNotNull(t);
			Assert.assertEquals("someText", row.get(0));
			Assert.assertEquals(t.getCode(), row.get(1));
			Assert.assertTrue(((String) row.get(1)).startsWith("Title-"));
			Assert.assertEquals(t.getCode(), row.get(2));
		}
	}

	private SearchResult query(final FlexibleSearch fs, final Query query)
	{
		return fs.search(//
				query.getQuery(),//
				query.getValues(), //
	...
	static class Query
	{
		...
		Query(final Query q)
		{
			this.dbQuery = q.dbQuery;
			this.values = q.values;
			this.resultClasses = q.resultClasses;
			this.failOnUnknownFields = q.failOnUnknownFields;
			this.dontNeedTotal = q.dontNeedTotal;
			this.start = q.start;
			this.count = q.count;
		}
