...
public class ImpExManagerTest extends AbstractImpExTest
{
	...
	public void testImportDataLightStream()
	{
		try
		{
			final InputStream is = ImpExManager.class.getResourceAsStream("/impex/testfiles/productFull.impex");
			assertNotNull("Can not read from jar file 'productFull.impex'", is);
			final Importer importer = ImpExManager.getInstance().importDataLight(is, windows1252.getCode(), false);
	...
	public void testImportDataLightReader()
	{
		try
		{
			final InputStream is = ImpExManager.class.getResourceAsStream("/impex/testfiles/productFull.impex");
			assertNotNull("Can not read from jar file 'productFull.impex'", is);
			final Importer importer = ImpExManager.getInstance().importDataLight(new CSVReader(is, windows1252.getCode()), false, 1);
			assertNotNull(importer);
			assertFalse(importer.hadError());
		}
	...
	public void testImportDataFull()
	{
		final InputStream is = ImpExManager.class.getResourceAsStream("/impex/testfiles/productFull.impex");
		assertNotNull("Can not read from jar file 'productFull.impex'", is);
		final ImpExImportCronJob cronJob = ImpExManager.getInstance().importData(is, windows1252.getCode(),
	...
	{
		final InputStream is = ImpExManager.class.getResourceAsStream("/impex/testfiles/product2Cycle.impex");
		assertNotNull("Can not read from jar file 'product2Cycle.impex'", is);
		final ImpExImportCronJob cronJob = ImpExManager.getInstance().importData(is, windows1252.getCode(),
				CSVConstants.HYBRIS_FIELD_SEPARATOR, CSVConstants.HYBRIS_QUOTE_CHARACTER, false);
		assertNull(cronJob);
		// clean up
	...
	{
		final InputStream is = ImpExManager.class.getResourceAsStream("/impex/testfiles/productUnresolved.impex");
		assertNotNull("Can not read from jar file 'productUnresolved.impex'", is);
		...
		try
		{
			...
			cronJob = ImpExManager.getInstance().importData(is, windows1252.getCode(), CSVConstants.HYBRIS_FIELD_SEPARATOR,
	...
	public void testImportDataLightNested()
	{
		try
		{
			final InputStream is = ImpExManager.class.getResourceAsStream("/impex/testfiles/productFullNested.impex");
			assertNotNull("Can not read from jar file 'productFullNested.impex'", is);
			final Importer importer = ImpExManager.getInstance().importDataLight(new CSVReader(is, windows1252.getCode()), true, 1);
			assertNotNull(importer);
			assertFalse(importer.hadError());
		}
		catch (final UnsupportedEncodingException e1)
	...
	public void testExportImport()
	{
		...
		try
		{
			...
			p = ProductManager.getInstance().createProduct("myProduct");
			p.setName("myProductName");
	...
	private void closeInputStreamStream(final InputStream is)
	{
		if (is != null)
		{
			try
