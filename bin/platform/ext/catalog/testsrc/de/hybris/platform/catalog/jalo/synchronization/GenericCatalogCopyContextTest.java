		...
		Mockito.verify(flexibleSerachMock, Mockito.times(1)).search(Mockito.any(SessionContext.class),
				(Mockito.argThat(new ArgumentMatcher<String>()
				{
					...
					public boolean matches(final Object argument)
					{
						...
						Assert.assertEquals(
								"SELECT x.PK FROM ({{ SELECT {pk} AS PK FROM {mou} WHERE {ennie-minnie}=?tgtVer  AND {minnie} IS NULL }} UNION ALL {{ SELECT {pk} AS PK FROM {ItemSyncTimestamp*} WHERE {sourceItem}=?srcItem AND {targetVersion}=?tgtVer AND {syncJob}=0  }} ) x",
								query);
						return true;
					}
		...
		Mockito.verify(flexibleSerachMock, Mockito.times(1)).search(Mockito.any(SessionContext.class),
				(Mockito.argThat(new ArgumentMatcher<String>()
				{
					...
					public boolean matches(final Object argument)
					{
						...
						final String query = (String) argument;
						Assert.assertEquals(
								"SELECT x.PK FROM ({{SELECT {pk} AS PK FROM {whou} WHERE  {minnie} IS NULL }} UNION ALL {{SELECT {pk} FROM {ItemSyncTimestamp*}WHERE {syncJob}=0 AND {targetVersion}=?tgtVer AND {sourceItem}=?srcItem}})x",
								query);
						return true;
					}
				})), Mockito.anyMap(), Mockito.any(Class.class));
