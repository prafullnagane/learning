	...
	private static class LocalizationFileGrabber
	{
		...
		public List<File> grabFiles()
		{
			...
			if (workingDirectory.isDirectory()
					&& Arrays.asList(workingDirectory.listFiles()).contains(new File(workingDir, "platform")))
			{
				final String[] locations = new String[]
				{ "platform", "ext-hybris", "ext-commerce", "ext-content", "ext-channel", "ext-accelerator", "ext-template",
						"ext-internal" };
				for (final String location : locations)
				{
					locationDir = new File(workingDir, location);
					(new FileLocator(locationDir)).executeProcess();
				}
			}
			if (!extensions.isEmpty())
			{
				...
				{
					locationDir = extension.getExtensionDirectory();
					(new FileLocator(locationDir)).executeProcess();
				}
				locationDir = new File(new File(workingDirectory, "platform"), "extgen");
				(new FileLocator(locationDir)).executeProcess();
			}
			else
			{
		...
		/**
		 * Can locate localized files with multi-thread support.
		 */
		private class FileLocator
		{
			private final File path;

			public FileLocator(final File path)
			{
				this.path = path;
			}
			...
			public void executeProcess()
			{
				...
				final File[] folderList = path.listFiles(new directoryFilter());
				path.listFiles(new xmlFileFilter());
				path.listFiles(new propertyAndZulFileFilter());
				path.listFiles(new csvAndImpExFileFilter());
				for (final File folder : folderList)
				{
					final FileLocator fileLoc = new FileLocator(folder);
					fileLoc.executeProcess();
					/**
					 * tPoolExecutor.execute(fileLoc);
					 */
				}
			}
			...
			private class propertyAndZulFileFilter implements FileFilter
			{
				@Override
				public boolean accept(final File file)
				{
			...
			private class csvAndImpExFileFilter implements FileFilter
			{
				@Override
				public boolean accept(final File file)
				{
