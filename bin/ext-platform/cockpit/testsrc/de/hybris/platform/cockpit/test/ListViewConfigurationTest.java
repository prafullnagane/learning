...
public class ListViewConfigurationTest extends UIConfigurationTestBase
{
	...
	private void assertHasLanguagesIfLocalized(final ColumnConfiguration config)
	{
		if (config instanceof PropertyColumnConfiguration)
		{
			...
			final PropertyDescriptor pd = propertyConfig.getPropertyDescriptor();
			if (pd.isLocalized() && pd.getQualifier().endsWith(".name"))
			{
				assertFalse("Property localized but has no languages: " + pd.getQualifier(), config.getLanguages().isEmpty());
			}
			else if (pd.isLocalized() && pd.getQualifier().endsWith(".description"))
			{
				assertTrue("Property configuration is not localized but has languages: " + pd.getQualifier(), config.getLanguages()
						.isEmpty());
			}
			else
			{
				assertTrue("Property not localized but has languages: " + pd.getQualifier(), config.getLanguages().isEmpty());
			}
		}
	}
}
