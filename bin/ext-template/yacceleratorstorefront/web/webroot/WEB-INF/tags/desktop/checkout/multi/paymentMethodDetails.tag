	...
	<td headers="header1">
		<ul class="table_action_col_item">
			...
			<c:choose>
				...
				<c:otherwise>
					<a id="paymentMethod_select_link_${count}" href="${selectPaymentMethodUrl}/?selectedPaymentMethodId=${paymentMethod.id}" class="select-link neutral"><spring:theme code="checkout.multi.paymentMethod.savedCards.select"/></a>
				</c:otherwise>
			</c:choose>
			</li>
		</ul>
	...
	<td headers="header3" >
		<ul>
			<c:if test="${billingAddressOk}">
				<li>${fn:escapeXml(paymentMethod.billingAddress.title)}&nbsp;${fn:escapeXml(paymentMethod.billingAddress.firstName)}&nbsp;${fn:escapeXml(paymentMethod.billingAddress.lastName)}</li>
