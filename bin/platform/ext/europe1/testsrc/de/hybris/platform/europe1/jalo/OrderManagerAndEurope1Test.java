...
public class OrderManagerAndEurope1Test extends HybrisJUnit4Test
{
	...
	public void setUp() throws Exception
	{
		...
		assertNotNull(eu = c2lm.createCurrency("om-eu"));
		oldCurr = jaloSession.getSessionContext().getCurrency();
		jaloSession.getSessionContext().setCurrency(eu);

		//eu.setActive( true );
		eu.setSymbol("E");
		eu.setDigits(2);
		eu.setConversionFactor(1);
	}
	...
	public void testAll() throws Exception
	{
		...
		//deC.setActive( true );
		assertNotNull(dirk = um.createCustomer("dirk"));
		...
		assertNotNull(tom = um.createCustomer("tom"));
		...
		assertNotNull(uPs = pm.createUnit("pieces", "pieces"));
		uPs.setConversionFactor(1.0);
		assertNotNull(copylifepapier = pm.createProduct("10011"));
		copylifepapier.setUnit(uPs);
		assertNotNull(lampe = pm.createProduct("10012"));
		lampe.setUnit(uPs);
		assertNotNull(laserfazer = pm.createProduct("10013"));
		laserfazer.setUnit(uPs);
		final Europe1PriceFactory pf = (Europe1PriceFactory) jaloSession.getOrderManager().getPriceFactory();
		assertNotNull(pf.createPriceRow(copylifepapier, null, null, null, 1, eu, uPs, 1, true, null, 4.90));
		assertNotNull(pf.createPriceRow(lampe, null, null, null, 1, eu, uPs, 1, true, null, 5.90));
		assertNotNull(pf.createPriceRow(laserfazer, null, null, null, 1, eu, uPs, 1, true, null, 6.90));
		assertNotNull(tax = om.createTax("tax"));
		tax.setValue(16);
		assertNotNull(pf.createTaxRow(null, null, null, null, tax, null, null));
		// Orders
		assertNotNull(order1 = om.createOrder("100010", dirk, eu, new Date(System.currentTimeMillis()), true));
		...
		assertNotNull(order2 = om.createOrder("100011", dirk, eu, new Date(System.currentTimeMillis()), true));
		...
		assertNotNull(order3 = om.createOrder("100012", tom, eu, new Date(System.currentTimeMillis()), true));
		...
		assertNotNull(cartOrder = om.createOrder(cart));
	...
	public void testTaxCaching() throws JaloInvalidParameterException, ConsistencyCheckException, JaloPriceFactoryException
	{
		...
		try
		{
			final Europe1PriceFactory e1 = Europe1PriceFactory.getInstance();
			jaloSession.getTenant().getConfig().setParameter(Europe1Constants.KEY_CACHE_TAXES, "true");
			e1.invalidateTaxCache();

			final Tax t1 = OrderManager.getInstance().createTax("FULL");
			final Tax t2 = OrderManager.getInstance().createTax("HALF");
			...
			final Product p2 = ProductManager.getInstance().createProduct("bar");
			final Unit unit = ProductManager.getInstance().createUnit("xxx", "yyy");


			e1.setEurope1PriceFactory_PTG(p, full);
			e1.setEurope1PriceFactory_PTG(p2, half);
			e1.setEurope1PriceFactory_UTG(u, userGrp1);

			final TaxRow tr1 = e1.createTaxRow(null, full, null, userGrp1, t1, null, Double.valueOf(19));
			final TaxRow tr2 = e1.createTaxRow(null, half, null, userGrp1, t2, null, Double.valueOf(7));
			final TaxRow tr3 = e1.createTaxRow(null, full, null, userGrp2, t1, null, Double.valueOf(25));
			final TaxRow tr4 = e1.createTaxRow(null, half, null, userGrp2, t2, null, Double.valueOf(10));

			final Cart cart = jaloSession.getCart();
			final CartEntry entry1 = (CartEntry) cart.addNewEntry(p, 2, unit);
			final CartEntry entry2 = (CartEntry) cart.addNewEntry(p2, 10, unit);

			Collection<TaxValue> taxes = e1.getTaxValues(entry1);
			assertEquals(Collections.singletonList(new TaxValue(t1.getCode(), 19, false, null)), taxes);
			taxes = e1.getTaxValues(entry2);
			assertEquals(Collections.singletonList(new TaxValue(t2.getCode(), 7, false, null)), taxes);

			e1.setEurope1PriceFactory_UTG(u, userGrp2);

			taxes = e1.getTaxValues(entry1);
			assertEquals(Collections.singletonList(new TaxValue(t1.getCode(), 25, false, null)), taxes);
			taxes = e1.getTaxValues(entry2);
			assertEquals(Collections.singletonList(new TaxValue(t2.getCode(), 10, false, null)), taxes);

			final TaxRow tr5 = e1.createTaxRow(null, null, null, null, t2, null, Double.valueOf(100));

			e1.setEurope1PriceFactory_PTG(p2, null);
			taxes = e1.getTaxValues(entry2);
			assertEquals(Collections.singletonList(new TaxValue(t2.getCode(), 100, false, null)), taxes);

			final TaxRow tr6 = e1.createTaxRow(p, null, null, null, t1, null, Double.valueOf(30));

			taxes = e1.getTaxValues(entry1);
			assertEquals(
					new HashSet(Arrays.asList(new TaxValue(t1.getCode(), 30, false, null),
							new TaxValue(t1.getCode(), 25, false, null), new TaxValue(t2.getCode(), 100, false, null))), new HashSet(
							taxes));

			final TaxRow tr7 = e1.createTaxRow(null, null, u, null, t1, null, Double.valueOf(40));

			taxes = e1.getTaxValues(entry2);
			assertEquals(
					new HashSet(Arrays.asList(new TaxValue(t1.getCode(), 40, false, null),
							new TaxValue(t2.getCode(), 100, false, null))), new HashSet(taxes));

			assertTrue(e1.isCachingTaxes());
			...
			assertEquals(Collections.EMPTY_LIST, e1.getTaxValues(entry1));
		}
		finally
		{
