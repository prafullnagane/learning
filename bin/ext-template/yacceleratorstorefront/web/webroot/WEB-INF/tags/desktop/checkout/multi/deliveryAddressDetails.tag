	...
	<td headers="header1">
		<ul class="table_action_col_item">
			...
			<c:choose>
				...
				<c:otherwise>
					<a id="deliveryAddress_select_link_${count}" href="${selectDeliveryAddressUrl}/?selectedAddressCode=${deliveryAddress.id}" class="select-link neutral"><spring:theme code="checkout.multi.deliveryAddress.select" text="Select"/></a>
				</c:otherwise>
			</c:choose>
			</li>
			<li>
