...
public class TaskSchedulingConcurrencyTest extends ServicelayerBaseTest
{
	...
	private static TaskModel createDummyTaskWithCondition(final String conditionUID, final Date executionDate)
	{
		final TaskModel task = new TaskModel();
		task.setRunnerBean("SomeBean");
		task.setExecutionDate(executionDate);
		final TaskConditionModel cond = new TaskConditionModel();
	...
	private static class TriggerEventWhileSchdulingTaskService extends DefaultTaskService
	{
		...
		{
			setModelService(ms);
			setScheduleAndTriggerStrategy(new DefaultScheduleAndTriggerStrategy()
			{
				{
					setModelService(ms);
				}
			...
			setTaskDao(new TaskDAO(Registry.getCurrentTenantNoFallback()));
	...
	//					 +-> try to match condition again -> now success
	//
	private static class ScheduleTaskWhileTriggerEventService extends DefaultTaskService
	{
		boolean retryCalled = false;
		Exception otherThreadError = null;
		PK createdTask;
