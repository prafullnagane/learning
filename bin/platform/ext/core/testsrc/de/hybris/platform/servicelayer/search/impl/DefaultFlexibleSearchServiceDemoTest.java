...
public class DefaultFlexibleSearchServiceDemoTest extends ServicelayerTransactionalBaseTest
{
	...
	{
		final String prefix = de.hybris.platform.util.Config.getString("db.tableprefix", "");
		...
		final String expectedTranslatedQuery = "SELECT  item_t0.PK  FROM " + prefix
				+ "languages item_t0 WHERE ( item_t0.ActiveFlag  = ?) AND (item_t0.TypePkString=? ) order by  item_t0.PK ";
		assertThat(translationResult.getSQLQuery()).isEqualTo(expectedTranslatedQuery);
