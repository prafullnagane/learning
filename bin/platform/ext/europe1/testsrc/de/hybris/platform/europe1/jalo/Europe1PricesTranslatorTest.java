...
public class Europe1PricesTranslatorTest extends HybrisJUnit4TransactionalTest
{
	...
	public void testOverallImport()
	{
		...
		try
		{
			final Product p1 = (Product) ir.readLine();
			assertNotNull(p1);
		...
		try
		{
			final Product p2 = (Product) ir.readLine();
			assertNotNull(p2);
		...
		try
		{
			final Product p3 = (Product) ir.readLine();
			assertNotNull(p3);
		...
		try
		{
			final Product p4 = (Product) ir.readLine();
			assertNotNull(p4);
			assertEquals("G02503", p4.getCode());
			assertEquals(4, epf.getEurope1Prices(p4).size());

			final Product p5 = (Product) ir.readLine();
			assertNotNull(p5);
			assertEquals(testProduct5.getCode(), p5.getCode());
			assertEquals(10, epf.getEurope1Prices(p5).size());
		}
		catch (final Exception e)
		{
			e.printStackTrace();
	...
	private void checkPrice(final NumberFormat nf, final String priceAsString, final PriceRow pr)
	{
		try
		{
			assertEquals(pr.getPriceAsPrimitive(), nf.parse(priceAsString).doubleValue(), 0.00);
		}
