...
public class ImpExMultThreadedTest extends HybrisJUnit4Test
{
	...
	public void setUp() throws JaloInvalidParameterException, JaloDuplicateCodeException, JaloItemNotFoundException
	{
		...
		varType = tm.createComposedType(tm.getComposedType("VariantProduct"), "MyVar");

		try
		{
			final AttributeDescriptor ad = varType.getAttributeDescriptorIncludingPrivate("approvalStatus");
			ad.setOptional(true);
		}
	...
	protected void checkMultiThreadedImport(final MultiThreadedImporter mImp, final int max, final Set<Product> products)
	{
		...
		for (int i = 0; i < max; i++)
		{
			assertNotNull(pm.getProductsByCode("product-2-" + i));
		}
		assertNotNull(pm.getProductsByCode("product-2-base"));
		assertNotNull(pm.getProductsByCode("product-2-var-3"));
		assertNotNull(pm.getProductsByCode("product-2-var-2"));
	}
	...
	public void testMinimalThreadSetup() throws ImpExException
	{
		...
		final Set<Product> p2 = new LinkedHashSet<Product>(MAX3);
		...
		LOG.info("done in " + (t4 - t3) + " ms after " + mImp.getCurrentPass() + " passes");

		checkMultiThreadedImport(mImp, MAX3, p2);
	}
	...
	public void testImport() throws ImpExException
	{
		...
		final Set<Product> p1 = new LinkedHashSet<Product>(MAX);
		...
		LOG.info("done in " + (t2 - t1) + " ms after " + imp.getCurrentPass() + " passes. Rate is " + ((MAX * 1000) / (t2 - t1))
				+ " items/s");
		...
		final Set<Product> p2 = new LinkedHashSet<Product>(MAX * MULTI_FACTOR);
		...
		final long t4 = System.currentTimeMillis();
		LOG.info("done in " + (t4 - t3) + " ms after " + mImp.getCurrentPass() + " passes. Rate is "
				+ ((MAX * MULTI_FACTOR * 1000) / (t4 - t3)) + " items/s");

		checkMultiThreadedImport(mImp, MAX * MULTI_FACTOR, p2);

		LOG.info("Difference single-multi = " + (((t4 - t3) * 100) / (MULTI_FACTOR * (t2 - t1))) + " % ");
	}
