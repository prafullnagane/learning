...
public class VariantsTest extends ServicelayerTransactionalTest
{
	...
	public void setUp() throws Exception
	{
		...
		assertNotNull(shirtSizeEnumType = em.createEnumerationType("shirtSize", null));
		assertNotNull(s = em.createEnumerationValue(shirtSizeEnumType, "s"));
		assertNotNull(m = em.createEnumerationValue(shirtSizeEnumType, "m"));
		assertNotNull(l = em.createEnumerationValue(shirtSizeEnumType, "l"));
		assertNotNull(xl = em.createEnumerationValue(shirtSizeEnumType, "xl"));
		assertNotNull(xxl = em.createEnumerationValue(shirtSizeEnumType, "xxl"));
	...
	public void testAttributeAccessorClash() throws JaloInvalidParameterException, JaloBusinessException
	{
		...
		assertNotNull(v1 = createShoe(base1, base1.getCode() + " juhu", "trallal", "jaja"));
		assertNotNull(v2 = createShirt(base2, base2.getCode() + " juhu", xxl, "jaja"));

		assertEquals(shoeType, v1.getComposedType());
		assertEquals(shirtType, v2.getComposedType());

		assertNull(v1.getAttribute("foo"));
		assertNull(v2.getAttribute("foo"));

		v1.setAttribute(ctx, "foo", new Double(123.4));
		// setting single value using session language shouldn't be a problem
		v2.setAttribute("foo", new Double(123.4));
		assertEquals(new Double(123.4), v2.getAttribute("foo"));
		...
		try
		{
			...
			v2.setAttribute(ctx, "foo", new Double(123.4));
		...
		v2.setAttribute(ctx, "foo", values);
		...
		assertEquals(new Double(123.4), v1.getAttribute("foo"));
		assertEquals(new Double(123.4), v2.getAttribute("foo")); // using session context lang
		assertEquals(values, v2.getAttribute(ctx, "foo"));
	}
