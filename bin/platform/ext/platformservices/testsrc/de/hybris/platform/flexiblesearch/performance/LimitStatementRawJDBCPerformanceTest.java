...
public class LimitStatementRawJDBCPerformanceTest extends ServicelayerTransactionalTest
{
	...
	{
		return JaloSession.getCurrentSession().getTenant().getDataSource();
	}
	...
	{
		final String deleteQuery = "TRUNCATE TABLE junit_titles";
	...
	private void writeResultToLogger(final String formattedTime, final int start, final int count)
	{
		...
		final StringBuilder sb = new StringBuilder(endLine);
		sb.append(line).append(Config.getDatabase()).append(line).append(endLine);
		sb.append("Resulting time for - ").append(start).append(", ");
		sb.append(count).append(" :").append(formattedTime).append(endLine);
